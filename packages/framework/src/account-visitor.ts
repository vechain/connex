    ...
    return {
        ...
        method: jsonABI => {
            ...
            try {
                ...
                throw new R.BadParameter(`arg0: expected valid ABI (${(err as Error).message})`)
            }
    ...
    return {
        asCriteria: indexed => {
            try {
                ...
                throw new R.BadParameter(`arg0: can not be encoded (${(err as Error).message})`)
            }
        },
        filter: (indexed) => {
            R.test(indexed, [{}], 'arg0')
            ...
            const criteria = indexed.map((o, i) => {
                try {
                    ...
                    throw new R.BadParameter(`arg0.#${i}: can not be encoded (${(err as Error).message})`)
                }
            })
