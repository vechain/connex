...
export const createSync2: Connex.NewSigner = async (genesisId:string) => {
...
export const createSync: Connex.NewSigner = async (genesisId: string) => { 
        ...
        signTx: (msg: Connex.Vendor.TxMessage, options: Connex.Signer.TxOptions): Promise<Connex.Vendor.TxResponse> => {
            ...
            if (options.delegator) {
                s1.delegate(async (unsignedTx) => {
                    const res = await fetch(options.delegator!.url, {
                        method: 'POST',
...
export const createVeworldExtension: Connex.NewSigner = async (genesisId: string) => { 
